...
from app.models import GlobalLLMConfig
...
from src.forecast.sarima_model import SarimaForecaster
from src.llm.client import LLMClient, LLMConfig
from src.preprocess import clean_monthly_data, compute_basic_statistics
...
class NationalService:
    def __init__(self, runtime: AppRuntimeState, db: Session) -> None:
    ...
    def run_forecast(self, payload: NationalForecastRunRequest) -> dict[str, Any]:
        ...
        report_text = generate_report(history_df=clean_df, forecast_df=forecast_df, stats=stats, llm_client=None)
    ...
    def test_llm(self, payload: NationalLLMTestRequest) -> dict[str, Any]:
        llm_client = self._build_llm_client(payload.llm_config)
    ...
    def upsert_llm_config(self, payload: NationalLLMConfigPayload) -> dict[str, Any]:
        config = get_global_llm_config(self.db)
        values = {
        ...
        return {"config": self._serialize_global_config(config)}

    def get_llm_config(self) -> dict[str, Any]:
        return {"config": self._serialize_global_config(get_global_llm_config(self.db))}
    ...
    def _build_llm_client(self, payload: NationalLLMConfigPayload | None) -> LLMClient:
        ...
        config = LLMConfig(
            api_key=active_payload.api_key,
            base_url=active_payload.base_url,
            model=active_payload.model,
            provider=active_payload.provider,
        )
        return LLMClient(enabled=active_payload.enabled, config=config)
    ...
    def _serialize_llm_config(self, payload: NationalLLMConfigPayload) -> dict[str, Any]:
        masked_api_key = ""
        if payload.api_key:
    ...
    def _serialize_global_config(self, config: GlobalLLMConfig | None) -> dict[str, Any]:
        return self._serialize_llm_config(self._global_config_to_payload(config))

    def _global_config_to_payload(self, config: GlobalLLMConfig | None) -> NationalLLMConfigPayload:
        if config is None:
            return NationalLLMConfigPayload()
        return NationalLLMConfigPayload(
