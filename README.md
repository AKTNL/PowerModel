...
```text
base_prediction = 0.5 * 最近一个月用电量
                + 0.3 * 近三个月平均用电量
                + 0.2 * 去年同期用电量或近三个月均值兜底

predicted_kwh = base_prediction * 季节因子 * 家庭画像修正因子
```
...
- 去年同期用电量：`235 kWh`
- 目标月份为夏季，季节因子：`1.08`
- 家庭画像中空调数量不少于 `2` 台，额外修正：`1.03`

则：

```text
base_prediction = 0.5 * 260 + 0.3 * 240 + 0.2 * 235
                = 249 kWh

predicted_kwh = 249 * 1.08 * 1.03
              ≈ 276.99 kWh
```

如果系统估算电价为 `0.56 元/kWh`，则预计电费约为：

```text
predicted_bill = 276.99 * 0.56
               ≈ 155.11 元
```

预测区间使用 `max(15, predicted_kwh * 0.12)` 作为波动范围，因此该例的区间约为：

```text
lower_bound = 276.99 - 33.24 ≈ 243.75 kWh
upper_bound = 276.99 + 33.24 ≈ 310.23 kWh
```
