...
- `数值预测`：后端负责家庭侧可解释规则加权预测与国家级 SARIMA 时间序列预测；家庭模块支持目标月温度/节假日上下文输入、结构化贡献拆解与反事实模拟。
- `智能解释`：OpenAI 兼容大模型用于报告润色、预测解释、节能建议和问答，但数值贡献和模拟结果都由本地预测逻辑生成。
...
- 家庭下月用电量、电费区间、结构化贡献拆解和节能建议生成。
- 家庭预测支持目标月平均温度、节假日天数等上下文输入，并展示参考值对照、贡献瀑布链路和贡献卡片。
- 情景模拟支持空调时长、空调设定温度、热水器模式、外出天数等反事实调节项，并输出结构化变化贡献。
...
- 家庭用电预测：使用本地规则加权模型。算法要求至少 `3` 个月历史用电数据，先计算由 `最近一个月用电量`、`近三个月平均值`、`去年同期用电量或近三月均值兜底` 组成的历史基线，再按情况叠加两类修正：
  1. 如果未填写目标月上下文，则继续使用夏季/冬季季节规则与空调数量修正；
  2. 如果填写了 `目标月平均温度`、`目标月节假日天数`，则输出更细的 `温度贡献`、`节假日贡献`、`空调保有量贡献`。
  最终同时返回预测区间、预测假设、结构化贡献项和前端可视化拆解结果，适合 Demo 中展示“为什么预测会上升或下降”。
...
```text
baseline_kwh = 0.5 * 最近一个月用电量
             + 0.3 * 近三个月平均用电量
             + 0.2 * 去年同期用电量或近三个月均值兜底
```

如果没有填写目标月上下文，系统会继续走季节规则：

```text
predicted_kwh = baseline_kwh + 季节修正 + 空调数量修正
```

如果填写了目标月上下文，则系统会优先走贡献拆解路径：

```text
predicted_kwh = baseline_kwh
              + 温度贡献
              + 节假日贡献
              + 空调保有量贡献
```

例如目标月份是 `2026-07`，历史基线约为 `262.4 kWh`，并且填写：
...
- 参考值对照：`目标温度 vs 参考温度`、`目标节假日 vs 参考节假日`
- 贡献瀑布链路：`历史基线 -> 各修正项 -> 最终预测`
- 贡献卡片：逐项解释每个 kWh 变化来自哪里

预测区间仍使用 `max(15, predicted_kwh * 0.12)` 作为波动范围。

### 国家预测算法示例

国家模块面向月度时间序列，先将官方或上传 CSV 中的 `date` 与 `consumption_billion_kwh` 清洗为按月频率的数据，再使用 `SARIMAX` 拟合 SARIMA 模型。
...
其中 `12` 表示月度数据的年度季节周期。模型拟合成功后，系统调用 `get_forecast(steps=预测月份数)` 生成未来月份预测，并使用 `alpha=0.2` 的置信区间生成上下界。最终前端会展示：
...
- `家庭 + 国家` 双尺度预测：同一个平台同时覆盖微观家庭用电和宏观国家月度用电趋势，既能展示个人用电决策，也能展示能源宏观趋势分析。
- `数值预测 + 大模型解释` 分层架构：预测结果由可解释的规则模型和 SARIMA 模型生成，大模型只负责报告润色、原因分析、节能建议和问答，避免把数值预测黑箱化。
- 共享模型配置：家庭模块和国家模块复用同一份 OpenAI 兼容模型配置，减少重复配置，也方便统一诊断、保存和回退。
- 规则兜底机制：即使外部大模型不可用，系统仍可完成预测、本地报告和规则问答，保证演示流程不中断。
- 数据来源可追溯：国家模块保留字段说明、官方来源、原始数据和清洗后数据预览，便于解释预测数据从哪里来、如何被处理。
- 前后端一体化演示闭环：从数据录入、CSV 校验、预测运行、图表展示、报告生成到智能问答，都在一个浅色数据工作台里完成。
...
3. 点击 `上传记录`。
4. 进入 `预测与建议`，点击 `运行预测`。
   可选填写 `目标月平均温度` 和 `目标月节假日天数`，让系统输出更细的温度贡献、节假日贡献和空调保有量贡献。
5. 如有模型 API，进入 `模型设置`，测试并保存平台模型配置。
6. 回到 `预测与建议`，点击 `重新生成建议`。
7. 进入 `情景模拟`，调整 `空调时长 / 空调设定温度 / 热水器模式 / 外出天数`，查看反事实模拟结果与变化贡献。
8. 进入 `智能问答` 继续演示。
...
{
  "user_id": 1,
  "reduce_ac_hours_per_day": 1.0,
  "ac_setpoint_delta_c": 1.0,
  "water_heater_mode": "timer",
  "away_days": 3
}
```
